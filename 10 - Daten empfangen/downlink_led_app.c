...
void app_init( void )
{
  // Init the LED depending hardware
  ...
  if( app_settings.u8_app_led_status != 0 ) // Check LED status
  {
    HW_GPIO_Write(GPIOA, GPIO_PIN_5, 1);  // Set LED on
  }
  else
  {
    HW_GPIO_Write(GPIOA, GPIO_PIN_5, 0);  // Set LED off
  }
